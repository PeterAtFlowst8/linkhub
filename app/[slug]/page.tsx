...
// Revalidate every 60s — fast updates without being fully dynamic
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: {
      ...
      description: data.tagline || `Links for ${data.name}`,
      type: 'profile',
      ...(data.avatar || data.logo ? { images: [data.avatar || data.logo || ''] } : {}),
    },
    twitter: {
      card: 'summary',
      title: data.name,
...
// SVG social icons — crisp at any size
const SOCIAL_ICONS: Record<string, { icon: string; label: string }> = {
  ...
  phone: {
    icon: '<path d="M20.01 15.38c-1.23 0-2.42-.2-3.53-.56a.977.977 0 00-1.01.24l-1.57 1.97c-2.83-1.35-5.48-3.9-6.89-6.83l1.95-1.66c.27-.28.35-.67.24-1.02-.37-1.11-.56-2.3-.56-3.53 0-.54-.45-.99-.99-.99H4.19C3.65 3 3 3.24 3 3.99 3 13.28 10.73 21 20.01 21c.71 0 .99-.63.99-1.18v-3.45c0-.54-.45-.99-.99-.99z"/>',
    label: 'Phone',
  },
  whatsapp: {
...
function SocialIcon({ platform, url }: { platform: string; url: string }) {
  ...
  return (
    <a
      ...
      className="social-icon opacity-50 hover:opacity-100"
      title={data.label}
    ...
    >
      <svg
        ...
        aria-hidden="true"
      >
        <g dangerouslySetInnerHTML={{ __html: data.icon }} />
      </svg>
    </a>
  );
}
...
export default async function LinkHubPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <div
      className="min-h-screen flex flex-col items-center justify-start px-5 py-14 sm:py-20"
    ...
    >
      <div className="w-full max-w-[440px] mx-auto flex flex-col items-center">
        ...
        {(data.avatar || data.logo) && (
          <div
            className="w-[88px] h-[88px] rounded-full overflow-hidden ring-2 ring-offset-2 animate-in"
            style={{
              ['--tw-ring-color' as never]: accent,
              ['--tw-ring-offset-color' as never]: bg,
            }}
          >
            <img
        ...
        {/* Name + Tagline */}
        <div className="text-center mt-5 mb-8 animate-in" style={{ animationDelay: '0.1s' }}>
          <h1 className="text-xl font-bold tracking-tight leading-tight">{data.name}</h1>
          {data.tagline && (
            <p className="text-[13px] mt-1.5 opacity-60 leading-relaxed">{data.tagline}</p>
          )}
        </div>
        ...
        <div className="w-full flex flex-col gap-3">
          {data.links?.map((link, i) => (
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="link-card block w-full text-center py-3.5 px-5 rounded-2xl font-medium text-[14px] tracking-wide animate-in"
              style={{
        ...
        {data.socials && data.socials.length > 0 && (
          <div
            className="flex items-center gap-5 mt-8 animate-in"
            style={{ animationDelay: `${0.15 + (data.links?.length || 0) * 0.05 + 0.1}s`, color: text }}
          >
        ...
        {data.footerText && (
          <p
            className="text-[11px] opacity-20 mt-10 tracking-wide animate-in"
            style={{ animationDelay: `${0.15 + (data.links?.length || 0) * 0.05 + 0.2}s` }}
          >
            {data.footerText}
          </p>
        )}
